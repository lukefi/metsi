...
class StateTreeTest(unittest.TestCase):
    ...
    def test_add_branch(self):
        ...
        state_tree.treatment_params = parameters
        state_tree.done_treatment = dummy_operation
        ...
        new_branch.treatment_params = parameters2
        new_branch.done_treatment = dummy_operation
        new_branch.time_point = 2

        state_tree.add_branch(new_branch)

        self.assertListEqual(state_tree.state, [2, 2, 2])
        self.assertEqual(state_tree.done_treatment, dummy_operation)
        self.assertEqual(state_tree.time_point, 1)
        self.assertDictEqual(state_tree.treatment_params, parameters)
        self.assertEqual(len(state_tree.branches), 1)

        self.assertListEqual(state_tree.branches[0].state, [4, 4, 4])
        self.assertEqual(state_tree.branches[0].done_treatment, dummy_operation)
        self.assertEqual(state_tree.branches[0].time_point, 2)
        self.assertDictEqual(state_tree.branches[0].treatment_params, parameters2)
    ...
    def test_write_to_file(self):
        ...
        state_tree.treatment_params = parameters1
        state_tree.done_treatment = dummy_operation

        branch = StateTree()
        branch.state = state2
        branch.treatment_params = parameters2
        branch.done_treatment = dummy_operation
        ...
        self.assertEqual(read_tree.done_treatment, dummy_operation)
        self.assertDictEqual(read_tree.treatment_params, {"increment": 2, "mult": 2})

        self.assertEqual(len(read_tree.branches), 1)
        self.assertListEqual(read_tree.branches[0].state, [1, 1, 1])
        self.assertEqual(read_tree.branches[0].done_treatment, dummy_operation)
        self.assertDictEqual(read_tree.branches[0].treatment_params, {"increment": 1, "mult": 1})
