...
class ForestryOperationsTest(unittest.TestCase):
    ...
    def test_stem_wood_biomass_vol_1(self):
        tree=ReferenceTree(breast_height_diameter=29.4,breast_height_age=31,height=18.1,species=2)
    ...
    def test_biomass_component_models_against_OPEMOTTI_sums(self):
        ...
        for j in range(0,len(df["LPM"])):
            stem_biomass = biomass.stem_wood_biomass_vol_2(stand.reference_trees[j],stand,df["Tilavuus"][j],df["Hukka"][j])
    ...
    def test_tree_biomass_models3(self):
        ...
        for i in range(0,len(df["LPM"])):
            ...
            biomass_totals_models.append(sum(biomass.tree_biomass(stand.reference_trees[i],stand,df["Tilavuus"][i],df["Hukka"][i],3)))
    ...
    def test_tree_biomass_models4(self):
        ...
        biomass_totals_models = []
        for i in range(0,len(df["d"])):
            biomass_totals_MELA.append(df["runko"][i]+df["kuori"][i]+df["elavat oksat"][i]+df["kuolleet oksat"][i]+df["lehdet"][i]+df["kanto"][i]+df["juuret 2 mm"][i])
    ...
    def test_tree_biomass_models2(self):
        tree=ReferenceTree(breast_height_diameter=29.4,breast_height_age=31,height=18.1,species=2,lowest_living_branch_height=2.715)
        stand = ForestStand(degree_days=1150)
        volume = 0.54
