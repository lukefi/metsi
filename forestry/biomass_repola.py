...
def stem_wood_biomass_vol_MELA_2(tree: ReferenceTree,stand: ForestStand,treevolume) -> list:
    ...
    if(tree.species ==1 ):
         ...
         stem_with_bark_bm=treevolume*(442.03-0.904*stump_diameter(tree)-82.695*tree.breast_height_diameter/tree.breast_height_age)         
    else: 
         stem_with_bark_bm=treevolume*(431.43+28.054*math.log(stump_diameter(tree))-52.203*tree.breast_height_diameter/tree.breast_height_age) 
    stem_bark = (stem_bark_biomass_1(tree)/(stem_wood_biomass_1(tree)+stem_bark_biomass_1(tree)))*stem_with_bark_bm
...
def stem_bark_biomass_1(tree:ReferenceTree) -> float:
    if tree.species==1: #Scots Pine
        ...
        lnbm=-4.548+9.448*(stump_diameter(tree)/(stump_diameter(tree)+18))+0.436*(math.log(tree.height))+(0.023+0.041)/2
        bm=math.exp(lnbm)
    else: #Others, model for Birch
        lnbm=-5.401+10.061*(stump_diameter(tree)/(stump_diameter(tree)+12))+2.657*(tree.height/(tree.height+20))+(0.01043+0.04443)/2
        bm=math.exp(lnbm)
    #bm = Biomass of component, kg
    #to tons:
    bm=bm/1000
...
def stem_bark_biomass_2(tree:ReferenceTree) -> float:
    if tree.species==1: #Scots Pine
        ...
        lnbm=-4.437+10.071*(stump_diameter(tree)/(stump_diameter(tree)+18))+0.261*(math.log(tree.height))+(0.019+0.039)/2
        bm=math.exp(lnbm)
    else: #Others, model for Birch
        lnbm=-5.433+10.121*(stump_diameter(tree)/(stump_diameter(tree)+12))+2.647*(tree.height/(tree.height+20))+(0.011+0.0044)/2
        bm=math.exp(lnbm)
    #bm = Biomass of component, kg
    #to tons:
    bm=bm/1000
...
def living_branches_biomass_1(tree:ReferenceTree) -> float:
    if tree.species==1: #Scots Pine
        ...
        lnbm=-4.214+14.508*(stump_diameter(tree)/(stump_diameter(tree)+13))-3.277*(tree.height/(tree.height+5))+(0.039+0.081)/2
        bm=math.exp(lnbm)
    else: #Others, model for Birch
        lnbm=-4.152+15.874*(stump_diameter(tree)/(stump_diameter(tree)+16))-4.407*(tree.height/(tree.height+10))+(0.02733+0.07662)/2
        bm=math.exp(lnbm)
    #bm = Biomass of component, kg
    #to tons:
    bm=bm/1000
...
def living_branches_biomass_2(tree:ReferenceTree) -> float:
    ...
    if tree.species==1: #Scots Pine
        ...
        lnbm=-2.945+12.698*(stump_diameter(tree)/(stump_diameter(tree)+14))-6.183*(tree.height/(tree.height+5))+0.959*math.log(cl)+(0.013+0.072)/2
        bm=math.exp(lnbm)
    else: #Others, model for Birch
        lnbm=-4.837+13.222*(stump_diameter(tree)/(stump_diameter(tree)+12))-4.639*(tree.height/(tree.height+12))+0.135*cl+(0.013+0.054)/2
        bm=math.exp(lnbm)
    #bm = Biomass of component, kg
    #to tons:
    bm=bm/1000
...
def foliage_biomass_1(tree: ReferenceTree) -> float:
    if tree.species==1: #Scots Pine
        ...
        lnbm=-2.994+12.251*(stump_diameter(tree)/(stump_diameter(tree)+10))-3.415*(tree.height/(tree.height+1))+(0.107+0.089)/2
        bm=math.exp(lnbm)
    else: #Others, model for Birch
...
def foliage_biomass_2(tree: ReferenceTree) -> float:
    ...
    cr =(tree.height-tree.lowest_living_branch_height)/tree.height
    if tree.species==1: #Scots Pine
        lnbm=-1.748+14.824*(stump_diameter(tree)/(stump_diameter(tree)+4))-12.684*(tree.height/(tree.height+1))+1.209*math.log(cl)+(0.032+0.093)/2
        bm=math.exp(lnbm)
    elif tree.species==2: #Norway Spruce
        lnbm=-0.085+15.222*(stump_diameter(tree)/(stump_diameter(tree)+4))-14.446*(tree.height/(tree.height+1))+1.273*math.log(cl)+(0.028+0.087)/2
        bm=math.exp(lnbm)
    else: #Others, model for Birch
...
def foliage_biomass_MOTTI(tree: ReferenceTree) -> float:
    ...
    cr =(tree.height-tree.lowest_living_branch_height)/tree.height
    if tree.species==1: #Scots Pine
        lnbm=-2.385+15.022*(stump_diameter(tree)/(stump_diameter(tree)+4))-11.979*(tree.height/(tree.height+1))+1.116*math.log(cl)+(0.034+0.095)/2
        bm=math.exp(lnbm)
    elif tree.species==2: #Norway Spruce
        lnbm=0.286+16.286*(stump_diameter(tree)/(stump_diameter(tree)+4))-15.576*(tree.height/(tree.height+1))+1.17*math.log(cl)+(0.021+0.09)/2
        bm=math.exp(lnbm)
    else: #Others, model for Birch
        lnbm=-20.856+22.320*(stump_diameter(tree)/(stump_diameter(tree)+2))+2.819*cr+(0.011+0.044)/2
        bm=math.exp(lnbm)
...
def roots_biomass_1(tree:ReferenceTree) -> float:
    if tree.species==1: #Scots Pine
        ...
        lnbm=-3.223+6.497*(stump_diameter(tree)/(stump_diameter(tree)+22))+1.033*math.log(tree.height)+(0.048+0.02677)/2
        bm=math.exp(lnbm)
    #bm = Biomass of component, kg
    #to tons:
    bm=bm/1000
...
def roots_biomass_MOTTI(tree:ReferenceTree) -> float:
    if tree.species==1: #Scots Pine
        ...
        lnbm=-3.183+7.204*(stump_diameter(tree)/(stump_diameter(tree)+22))+0.892*math.log(tree.height)+(0.047+0.027)/2
        bm=math.exp(lnbm)
    #bm = Biomass of component, kg
    #to tons:
    bm=bm/1000
