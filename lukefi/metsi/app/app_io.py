...
from lukefi.metsi.app.enum import (
    RunMode,
    StateFormat,
    StateInputFormat,
    StateOutputFormat,
...
class MetsiConfiguration(SimpleNamespace):
    ...
    def _convert_to_config(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if key not in config_enums:
                raise ConfigurationException(f"Invaldi app level configuration key '{key}'")
            if value and key in config_enums:
                try:
                    ...
                    raise ConfigurationException(f"Invalid value for {key}: {value}. Error: {e}")
            else:
                kwargs[key] = None 
        return kwargs
    ...
    def _validate_and_sort_run_modes(modes: list[str | RunMode]) -> list[RunMode]:
        ...
        if RunMode.EXPORT_PREPRO in sorted_modes and RunMode.PREPROCESS not in sorted_modes:
            raise ConfigurationException(f"Error: Run mode {RunMode.EXPORT_PREPRO} cannot be without {RunMode.PREPROCESS}")

        if RunMode.EXPORT in sorted_modes and not any(p in sorted_modes for p in [RunMode.SIMULATE, RunMode.POSTPROCESS]):
            raise ConfigurationException(f"Error: Run mode {RunMode.EXPORT} is possible only when {RunMode.SIMULATE} or {RunMode.POSTPROCESS} is included")
...
def generate_application_configuration(app_configurations: dict = {}) -> MetsiConfiguration:
...
def parse_cli_arguments(args: list[str]) -> dict:
    ...
    parser.add_argument('--resimulate', help='Path to a saved schedule (…/results/Stand_1/2) to re-run', default=None)
