...
def getseries(schedule: OperationPayload, name: str) -> CollectiveSeries:
    ...
    data, index = [], []
    for t,c in schedule.collected_data.operation_results["report_collectives"].items():
        if name in c:
            data.append(c[name])
            index.append(t)
...
def j_xda(out: IO, data: SimResults, xvariables: list[str]):
    ...
    collectives = {
        ...
        for c in payload.collected_data.operation_results["report_collectives"].values()
        for k in c
    }
