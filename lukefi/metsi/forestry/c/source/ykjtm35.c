...
static int check_triangle(int ind, double x, double y, int ykj)
{
    ...
    it1 = (t1 == 0) ||
          ((t1 > 0) == ((ptsx[1] - ptsx[0]) * (ptsy[2] - ptsy[0]) - (ptsy[1] - ptsy[0]) * (ptsx[2] - ptsx[0]) > 0));
    it2 = (t2 == 0) ||
          ((t2 > 0) == ((ptsx[2] - ptsx[1]) * (ptsy[0] - ptsy[1]) - (ptsy[2] - ptsy[1]) * (ptsx[0] - ptsx[1]) > 0));
    it3 = (t3 == 0) ||
          ((t3 > 0) == ((ptsx[0] - ptsx[2]) * (ptsy[1] - ptsy[2]) - (ptsy[0] - ptsy[2]) * (ptsx[1] - ptsx[2]) > 0));
...
int ykjtm35fin(double x, double y, double* u, double* v, int ykjtotm35)
{
    ...
    w[0] = ((x - ptsx[1]) * c0x + (y - ptsy[1]) * c0y) / ((ptsx[0] - ptsx[1]) * c0x + (ptsy[0] - ptsy[1]) * c0y);
    w[1] = ((x - ptsx[0]) * c1x + (y - ptsy[0]) * c1y) / ((ptsx[1] - ptsx[0]) * c1x + (ptsy[1] - ptsy[0]) * c1y);
    w[2] = ((x - ptsx[0]) * c2x + (y - ptsy[0]) * c2y) / ((ptsx[2] - ptsx[0]) * c2x + (ptsy[2] - ptsy[0]) * c2y);
